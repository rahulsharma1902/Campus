...
<section>
    <div class="container col-lg-8">
        <div class="form my-4">
            <form action="/nominatestaff" method="POST">
                @csrf
            <div class="card card-dark">
            <div class="card-header">
                <h3 class="card-title">SELECT YOUR STAFF OF THE WEEK</h3>
            </div>
            <div class="card-body">
               <div class="form-group">
                ...
                <select name="nomenation_id" id="nomenation_id" class="form-control">
                    ...
                    @foreach ( $staff as $st)
                        <option class="bg-dark" value="{{$st->user_id ?? ''}}">{{$st->name ?? ''}}</option>
                    @endforeach
                </select>
                <input type="hidden" name="college_id" value="{{$st->college_id ?? ''}}">
